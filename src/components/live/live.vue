	      ...
	      <swiper-slide class="on-tab-list" v-for="(onTabListBox,index) in msg" :key="index.key">
	        <ul class="on-tab-list-box">
	          ...
	          <li class="on-tab-list-box-list" v-for="onTabList in onTabListBox.msg" :key="onTabList.key">
	          	<router-link :to="'/live/details/this.userId/'+onTabListBox.type+'/'+onTabList.id" tag="a">
		            <div class="pic">
		            ...
		            <div class="live-more">
		            	<span class="live-more-btn btn2" v-if="index === 0&&onTabList.hasOrder==true">已预约</span>
		            	<span class="live-more-btn btn2" v-if="index === 0&&onTabList.hasOrder==false">预约观看</span>
		              <span class="live-more-btn btn1" v-if="index === 1"><span class="iconfont">&#xe62a;</span>正在直播</span>
...
//        console.log(JSON.stringify(res.data));
          ...
          console.log("error from 视频直播Listlive.vue");
        });
        //分页请求 End
      }
    },
...
//			url:"http://192.168.8.214:8443/applet/liveIndex.htm",
      method:"POST",
      params: {
        userId: this.userId,
        pageSize: this.pageSize,
