     ...
     <swiper-slide class="on-tab-list" v-for="(onTabListBox,index) in msg" :key="index.key">
       <ul class="on-tab-list-box">
         ...
         <li class="on-tab-list-box-list" v-for="onTabList in onTabListBox.msg" :key="onTabList.key">
           ...
           <router-link :to="'demoDetails/' + onTabList.worksId" append tag="div" class="pic" :style="{backgroundImage:'url('+ onTabList.coverFile +')'}"></router-link>
           ...
           <div class="demo-more">
             ...
             <router-link :to="'/demo/order/' + onTabList.designerId" tag="div" class="demo-more-btn" v-if="onTabList.designerId != 0">预约设计</router-link>
           </div>
...
export default {
   ...
   infiniteHandler($state,num) {
     if(this.msg[num].currentPage >= this.msg[num].totalPage){
       ...
       this.$http({
         ...
         console.log("error from 案例List demo.vue");
       });
       //分页请求 End
 ...
 mounted() {
   ...
   this.$http({
     ...
     console.log("error from 案例demo.vue");
   });
   //加载默认数据 Start
